...
describe('BookStaticDataService', () => {
  ...
  it('should return all books', inject(
    ...
    (service: BookDataService, backend: HttpTestingController) => {
      ...
      backend
        ...
        .flush(staticBookData, { status: 200, statusText: 'Ok' });
    }
  ));
  ...
  it('should return a single book', inject(
    ...
    (service: BookDataService, backend: HttpTestingController) => {
      // call service method and test IN the subscription. no need to use async anymore!!
      service.getBookByIsbn(staticBookData[0].isbn).subscribe((books) => {
        expect(books).toEqual(staticBookData[0]);
      });
      // Wait for the call and response with mockdata  `.flush()`
      backend
        .expectOne(`http://localhost:4730/books/${staticBookData[0].isbn}`)
        .flush(staticBookData[0], { status: 200, statusText: 'Ok' });
    }
  ));
  ...
  it('should create a book with a post', inject(
    ...
    (service: BookDataService, backend: HttpTestingController) => {
      // call service method and test IN the subscription. no need to use async anymore!!
      service.createBook(staticBookData[0]).subscribe((books) => {
        expect(books).toEqual(staticBookData[0]);
      });
      // Wait for the call and response with mockdata  `.flush()`
      backend
  ...
  it('should update a book with a patch', inject(
    ...
    (service: BookDataService, backend: HttpTestingController) => {
      // call service method and test IN the subscription. no need to use async anymore!!
      service
        .updateBook(staticBookData[0].isbn, staticBookData[0])
        .subscribe((books) => {
          expect(books).toEqual(staticBookData[0]);
        });
      // Wait for the call and response with mockdata  `.flush()`
      backend
        .expectOne({
          method: 'PATCH',
          url: `http://localhost:4730/books/${staticBookData[0].isbn}`,
        })
